...
(function ContactManager() {
  ...
  API = { // all functions return promise for the convenience of chainning further rendering operations
    ...
    createContact: async function(e) {
      let formData = new FormData($('form.create_contact')[0]),
    ...
    updateContact: async function(e) {
      let formData = new FormData($('form.edit_contact')[0]);
      formData.set('id', Contact.editingID);

      let json = this.processFormData(formData),
          jsonData = JSON.stringify(json),

          response = await fetch('http://localhost:3000/api/contacts/' + Contact.editingID, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: jsonData,
          })
    ...
    deleteContact: async function(e) {
      let response = await fetch('http://localhost:3000/api/contacts/' + Contact.editingID, {
        method: 'DELETE'
      })
    },
  ...
  App = {
    init: function() {
      Contact.debouncedSearch = debounce(Contact.search, 600, Contact)
