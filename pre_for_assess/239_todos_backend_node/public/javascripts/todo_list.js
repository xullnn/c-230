...
async function batchCreatingTodo(dataset) {
...
let App =  {
    init: async function() {
      ...
      this.currentSectionTitle = "All Todos"
    },
    ...
    updateSelectedAndCurrentSection: function() {
      if (this.lists.current_section.title === 'All Todos') {  // choose all todos
        ...
        this.lists.selected = Lists.sortTodosByCompletion(this.lists.done_todos_by_date[this.currentSectionTitle] || [])
      }
      else {
    ...
    markActive: function() {
      if (!this.inCompletedGroup) {
        if (this.currentSectionTitle === 'All Todos') {
          activeSection = document.querySelector("header#all_header")
        } else {
          activeSection = [...document.querySelectorAll('#all dl')].find(el => el.getAttribute('data-title') === this.currentSectionTitle)
        }
      } else {
        if (this.currentSectionTitle === 'Completed') {
          activeSection = document.querySelector("header#all_done_header")
        } else {
          activeSection = [...document.querySelectorAll('#completed_items dl')].find(el => el.getAttribute('data-title') === this.currentSectionTitle)
        }
      };
    ...
    handleCreateNewTodo: function(e) {   // create new form
      e.preventDefault();
      let currentSectionTitle = this.lists.current_section.title,
      activeSection;
    ...
    handleUpdateTodo: function(e) { // render edit form with id
      ...
      this.currentSectionTitle = this.lists.current_section.title;
      ...
      $('form').on('submit', (e) => {
        ...
        Todo.updateTodo(param, id).then(() => {$(".modal").fadeOut()}).then(() => {
          Lists.init()
            .then(lists => {this.lists = lists})
            .then(() => this.updateSelectedAndCurrentSection())
            .then(() => {this.initializeHTML()})
    ...
    handleClickOnSubSections: function(e) { // update Nav selected data set
      e.preventDefault();
      let dl = e.currentTarget,
          due = dl.getAttribute('data-title'),
          currentDl;

      if (dl.closest('section').id === "all") {
        this.inCompletedGroup = false;
    ...
    updateParitalSection: function(parentNode, partial, data) {
      parentNode.innerHTML = '';
      parentNode.insertAdjacentHTML('beforeend', partial(data));
    },
  }
...
let Todo = {
    ...
    getAllTodos: function() { // return promise
...
let Lists = {
    ...
    sortTodosByCompletion: function(todos) {
